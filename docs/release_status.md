...
Agent libOS 1.6.0 is the current release line for the core Python runtime scope
...
- A manual native workflow can build self-contained 1.6.0 internal desktop
...
- RuntimeStore schema v8 is the only store format accepted by ordinary 1.6.0
...
- The `release-artifacts` CI job is configured to build one canonical 1.6.0
...
observation, not Agent libOS 1.6.0 release evidence, and supports no call-count,
...
- Git is a Python Runtime/model-tool surface only. It requires an existing
  ...
  GUI/HTTP surface, or real GitHub/GitLab API integration in 1.6.0.
