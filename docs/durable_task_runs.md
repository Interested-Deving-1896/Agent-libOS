...
Durable Task Runs, introduced in Agent libOS 1.1.0, remain the 1.6.0
...
purge is a Python Host/admin surface in 1.6.0; it is not offered to ordinary
CLI or GUI users.
...
Host/admin-only explicit payload purge remains Python-only in 1.6.0):
...
separate Task Run requirements or wait HTTP route in 1.6.0: the detail response
