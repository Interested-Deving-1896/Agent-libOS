...
| --- | --- | --- | --- |
| Python | Package declares 3.11–3.14 | Ubuntu on 3.11 and 3.14 for Python lanes, with four runtime shards and two benchmark shards per version; Windows on 3.11 for the complete deterministic matrix split into per-lane jobs, with eight deterministic file-weighted runtime shards, three provider shards, and four benchmark shards; a configured Ubuntu, macOS 14, and Windows Python 3.11 matrix runs the platform-marked host-filesystem-identity gate with skips forbidden | 3.12/3.13 are inside the declared range but are not separate root-runtime jobs; macOS behavior outside the targeted filesystem-identity gate remains an environment gate |
