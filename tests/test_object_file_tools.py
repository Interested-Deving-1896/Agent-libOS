...
class ObjectFileToolTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_copy_file_via_named_object_without_materializing_content_to_process(self) -> None:
        ...
        client = GuardedActionClient(
            actions=[
                {"action": "create_object_from_file", "name": object_name, "path": source},
                {"action": "write_object_to_file", "name": object_name, "path": target},
                {"action": "process_exit", "payload": {"copied": True, "object_name": object_name}},
            ],
        ...
        self.assertEqual(action_names, ["create_object_from_file", "write_object_to_file", "process_exit"])
        self.assertTrue(create_result["ok"])
        self.assertTrue(write_result["ok"])
        self.assertNotIn(sentinel, json.dumps(create_result, ensure_ascii=False))
        self.assertNotIn(sentinel, json.dumps(write_result, ensure_ascii=False))
        self.assertEqual((self.runtime.workspace_root / target).read_text(encoding="utf-8"), source_path.read_text(encoding="utf-8"))
    ...
    def test_object_file_tools_enforce_filesystem_and_object_capabilities(self) -> None:
        ...
        self.assertIn("lacks read", denied_read.error or "")

        self.runtime.filesystem.grant_path(pid, source, [CapabilityRight.READ], issued_by="test")
        created = self.runtime.tools.call(pid, "create_object_from_file", {"name": object_name, "path": source})
        self.assertTrue(created.ok)
        self.assertNotIn("capability checked", json.dumps(created.payload, ensure_ascii=False))
        ...
        self.assertIn("lacks write", denied_write.error or "")

        self.runtime.filesystem.grant_path(pid, target, [CapabilityRight.WRITE], issued_by="test")
        written = self.runtime.tools.call(pid, "write_object_to_file", {"name": object_name, "path": target})
        self.assertTrue(written.ok)
        self.assertEqual((self.runtime.workspace_root / target).read_text(encoding="utf-8"), "capability checked")
