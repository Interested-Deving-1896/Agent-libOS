...
class JsonRpcPrimitiveTests(unittest.TestCase):
    def test_manifest_validation_rejects_unsafe_endpoint_shapes(self) -> None:
        ...
        try:
            runtime.jsonrpc.register_endpoint_from_yaml_text(
                _manifest("valid", "https://api.example.test/jsonrpc"),
                actor="cli",
                require_capability=False,
            )
            invalid_cases = [
                _manifest("bad-http", "http://api.example.test/jsonrpc"),
                _manifest("bad-userinfo", "https://user:pass@example.test/jsonrpc"),
                _manifest("bad-fragment", "https://api.example.test/jsonrpc#secret"),
                _manifest("bad:colon", "https://api.example.test/jsonrpc"),
                _manifest("missing-class", "https://api.example.test/jsonrpc", rollback_class=None),
                _manifest("literal-header", "https://api.example.test/jsonrpc", literal_header=True),
                _manifest("bad-bool", "https://api.example.test/jsonrpc", state_mutation='"false"'),
                _manifest("dup-method", "https://api.example.test/jsonrpc", duplicate_method=True),
            ]
    ...
    def test_call_requires_method_capability_and_records_http_effect(self) -> None:
        with _jsonrpc_server() as server:
            ...
            try:
                ...
                self.assertEqual(result.result["method"], "demo.echo")
                self.assertEqual(server.requests[0]["authorization"], "Bearer secret-token")
                self.assertEqual(server.requests[0]["body"]["jsonrpc"], "2.0")
                self.assertEqual(server.requests[0]["body"]["method"], "demo.echo")
                self.assertNotIn("url", runtime.audit.trace()[-1].decision or {})
                self.assertNotIn("secret-token", json.dumps([record.__dict__ for record in runtime.audit.trace()]))
                effect = [item for item in runtime.store.list_external_effects() if item.provider == "jsonrpc"][0]
                self.assertEqual(effect.rollback_class.value, "no_rollback_required")
                self.assertFalse(effect.state_mutation)
                self.assertTrue(effect.information_flow)
            finally:
    ...
    def test_jsonrpc_errors_and_http_failures_are_structured_results(self) -> None:
        with _jsonrpc_server() as server:
            ...
            try:
                ...
                runtime.jsonrpc.register_endpoint_from_yaml_text(
                    _manifest("errors", server.url, with_header=False),
                    actor="cli",
                    require_capability=False,
                )
                ...
                self.assertEqual(rpc_error.status.value, "jsonrpc_error")
                self.assertEqual(rpc_error.error["code"], -32000)

                server.mode = "http_500"
                http_error = runtime.jsonrpc.call(pid, "errors", "echo", {})
                self.assertFalse(http_error.ok)
                self.assertEqual(http_error.status.value, "http_error")
                self.assertEqual(http_error.http_status, 500)
            finally:
                runtime.close()
    ...
    def test_missing_header_env_fails_before_http_attempt_and_one_shot_consumption(self) -> None:
        with _jsonrpc_server() as server:
            ...
            try:
                ...
                runtime.jsonrpc.register_endpoint_from_yaml_text(
                    _manifest("needs-token", server.url),
                    actor="cli",
                    require_capability=False,
                )
                cap = runtime.capability.grant_once(
                    pid,
                    "jsonrpc:needs-token:echo",
                ...
                self.assertEqual(runtime.store.get_capability(cap.cap_id).uses_remaining, 1)
            finally:
    ...
    def test_effectful_provider_without_classifier_fails_closed(self) -> None:
        ...
        try:
            ...
            runtime.jsonrpc.register_endpoint_from_yaml_text(
                _manifest("no-classifier", "https://api.example.test/jsonrpc", with_header=False),
                actor="cli",
                require_capability=False,
            )
    ...
    def test_syscall_bypasses_tool_table_but_not_capabilities(self) -> None:
        with _jsonrpc_server() as server:
            ...
            try:
                ...
                runtime.jsonrpc.register_endpoint_from_yaml_text(
                    _manifest("sys", server.url, with_header=False),
                    actor="cli",
                    require_capability=False,
                )
                ...
                self.assertTrue(result["ok"])
                self.assertNotIn("call_jsonrpc_method", runtime.process.get(pid).tool_table)
            finally:
                runtime.close()
    ...
    def test_checkpoint_restores_referenced_endpoint_and_reports_remote_effect(self) -> None:
        with _jsonrpc_server() as server:
            ...
            try:
                ...
                runtime.jsonrpc.register_endpoint_from_yaml_text(
                    _manifest("ckpt", server.url, with_header=False),
                    actor="cli",
                    require_capability=False,
                )
                ...
                self.assertEqual(restored["external_effect_summary"]["by_provider_operation"]["jsonrpc.call"], 1)
                self.assertEqual(runtime.jsonrpc.inspect_endpoint("ckpt", require_capability=False)["endpoint_id"], "ckpt")
            finally:
                runtime.close()
            ...
            try:
                ...
                _run_cli_json(
                    [
                        ...
                        "grant",
                        spawned["pid"],
                        "jsonrpc:cli-demo:echo",
                        "--rights",
                        "read",
                    ]
                )
                called = _run_cli_json(
                    [
                        "--db",
                        db,
                        "jsonrpc",
                ...
                self.assertEqual(registered["endpoint_id"], "cli-demo")
                self.assertEqual(listed[0]["endpoint_id"], "cli-demo")
                self.assertIsNone(listed[0]["url"])
                self.assertEqual(inspected["url"], server.url)
            ...
            def do_POST(self) -> None:  # noqa: N802 - stdlib callback name
                ...
                if owner.mode == "jsonrpc_error":
                    ...
                    response = {
                        "jsonrpc": "2.0",
                        "id": body.get("id"),
                        "result": {"method": body.get("method"), "params": body.get("params")},
                    }
...
def _manifest(
    ...
    url: str,
    *,
    rollback_class: str | None = "no_rollback_required",
    state_mutation: bool | str = False,
    with_header: bool = True,
    literal_header: bool = False,
    duplicate_method: bool = False,
) -> str:
    header = ""
    if literal_header:
        header = """
