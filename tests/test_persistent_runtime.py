    ...
    def test_static_tool_ids_survive_runtime_reopen(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertTrue(runtime.tools.call(pid, "get_current_time", {}).ok)
            finally:
                runtime.close()
            ...
            try:
                ...
                self.assertEqual(resolved.tool_id, tool_id)
                self.assertTrue(result.ok, result.error)
                self.assertEqual(
                    len([row for row in reopened.tools.list() if row["name"] == "get_current_time"]),
                    1,
                )
            finally:
                reopened.close()
