...
class CheckpointManagerTests(unittest.TestCase):
    ...
    def test_restore_recovers_process_subtree_objects_capabilities_and_cwd_only(self) -> None:
        ...
        try:
            ...
            self.assertEqual(restored.payload, {"version": 1})
            with self.assertRaises(Exception):
                runtime.memory.get_object_by_name(pid, "temp")
            self.assertEqual(runtime.process.get(pid).working_directory, ".")
            self.assertEqual(runtime.process.get(child).status, ProcessStatus.RUNNABLE)
            self.assertFalse(runtime.capability.check(pid, "test:temporary", CapabilityRight.READ))
            self.assertEqual(runtime.memory.get_object(unrelated, other_handle).payload, {"keep": True})
            self.assertEqual(result["restored_pids"], [pid, child])
        finally:
            runtime.close()
    ...
    def test_restore_preserves_append_only_history_and_reports_external_effects(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                self.assertGreaterEqual(len(runtime.audit.trace()), after_write_audit_count + 1)
    ...
    def test_checkpoint_capabilities_gate_inspect_restore_and_fork(self) -> None:
        ...
        try:
            ...
            self.assertEqual(runtime.checkpoint.inspect(checkpoint_id, actor=owner)["checkpoint"]["pid"], owner)
            ...
            self.assertEqual(runtime.checkpoint.restore(owner, checkpoint_id)["status"], "restored")
            ...
            self.assertNotEqual(forked["fork_root_pid"], owner)
        finally:
            runtime.close()
    ...
    def test_restore_supersedes_post_checkpoint_mailbox_and_cancels_human_requests(self) -> None:
        ...
        try:
            ...
            self.assertEqual(runtime.store.get_process_message(message.message_id).status, ProcessMessageStatus.SUPERSEDED_BY_RESTORE)
            self.assertEqual(runtime.messages.unread(pid), [])
            self.assertEqual(runtime.human.get(request_id).status, HumanRequestStatus.CANCELLED)
            self.assertEqual(result["superseded_messages"], [message.message_id])
            self.assertEqual(result["cancelled_human_requests"], [request_id])
        finally:
            runtime.close()
    ...
    def test_fork_from_checkpoint_remaps_process_namespace_objects_and_capabilities(self) -> None:
        ...
        try:
            ...
            self.assertNotEqual(fork_pid, pid)
            self.assertNotEqual(fork_obj.oid, original.oid)
    ...
    def test_checkpoint_restores_registered_jit_tool_source(self) -> None:
        ...
        try:
            ...
            self.assertEqual(runtime.tools._jit_sources[handle.tool_id], source)
            self.assertEqual(runtime.tools.resolve("echo_value", pid=pid).tool_id, handle.tool_id)
            ...
            self.assertEqual(runtime.tools._jit_sources[handle.tool_id], source)
            self.assertEqual(runtime.tools.resolve("echo_value", pid=forked["fork_root_pid"]).tool_id, handle.tool_id)
        finally:
            runtime.close()
    ...
    def test_checkpoint_syscalls_use_primitive_capabilities(self) -> None:
        ...
        try:
            ...
            self.assertEqual(inspected["checkpoint"]["pid"], pid)
            with self.assertRaises(CapabilityDenied):
    ...
    def test_checkpoint_safe_point_normalizes_running_status(self) -> None:
        ...
        try:
            ...
            self.assertEqual(inspected["processes"][0]["status"], ProcessStatus.RUNNABLE.value)
    ...
    def test_default_images_expose_only_low_risk_checkpoint_tools(self) -> None:
        ...
        try:
            ...
            self.assertTrue(runtime.tools.call(pid, "create_checkpoint", {"reason": "tool"}).ok)
            self.assertIn("create_checkpoint", runtime.process.get(pid).tool_table)
            self.assertIn("inspect_checkpoint", runtime.process.get(pid).tool_table)
            self.assertIn("diff_checkpoint", runtime.process.get(pid).tool_table)
            self.assertIn("list_checkpoints", runtime.process.get(pid).tool_table)
            self.assertNotIn("restore_checkpoint", runtime.process.get(pid).tool_table)
            self.assertNotIn("fork_checkpoint", runtime.process.get(pid).tool_table)
        finally:
            runtime.close()
    ...
    def test_checkpoint_cli_outputs_stable_json(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(created["checkpoint_id"].startswith("ckpt_"))
            self.assertEqual(listed[0]["checkpoint_id"], created["checkpoint_id"])
            self.assertEqual(inspected["checkpoint"]["pid"], spawned["pid"])
