...
def test_desktop_release_names_are_exact_for_three_native_targets() -> None:
    assert finalizer._package_names("1.6.0", "darwin-arm64") == (
        "Agent-libOS-1.6.0-macos-arm64.dmg",
        "Agent-libOS-1.6.0-macos-arm64.zip",
    )
    assert finalizer._package_names("1.6.0", "win32-x64") == (
        "Agent-libOS-Setup-1.6.0-windows-x64.exe",
        "Agent-libOS-1.6.0-windows-x64.zip",
    )
    assert finalizer._package_names("1.6.0", "linux-x64") == (
        "Agent-libOS-1.6.0-linux-x64.AppImage",
        "Agent-libOS-1.6.0-linux-x64.tar.gz",
    )
...
def test_editable_checkout_receipt_is_removed_from_frozen_metadata(tmp_path: Path) -> None:
    metadata = tmp_path / "_internal" / "agent_libos-1.6.0.dist-info"
    ...
    (metadata / "RECORD").write_text(
        "agent_libos-1.6.0.dist-info/direct_url.json,sha256=abc,1\n"
        "agent_libos-1.6.0.dist-info/METADATA,sha256=def,2\n",
        encoding="utf-8",
    )
...
def test_desktop_finalizer_removes_unpublished_update_and_debug_metadata(
    ...
    for name in (
        "Agent-libOS-1.6.0-macos-arm64.zip.blockmap",
        "Agent-libOS-1.6.0-macos-arm64.dmg.blockmap",
        "builder-debug.yml",
