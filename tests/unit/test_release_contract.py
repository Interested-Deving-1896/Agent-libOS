...
def _write_release_pair(target: Path, *, version: str = "1.6.0") -> tuple[Path, Path]:
...
def test_release_version_identifiers_are_aligned() -> None:
    assert validate_version_alignment(ROOT) == "1.6.0"
...
def test_root_lock_resolves_the_reviewed_mcp_sdk_v2_graph() -> None:
    ...
    assert packages["agent-libos"]["version"] == "1.6.0"
...
def test_release_status_contains_current_version_state_only() -> None:
    ...
    assert text.startswith("# Agent libOS 1.6.0 Status\n")
    forbidden = {
...
def test_release_status_bounds_unarchived_evidence_and_volatile_counts() -> None:
    ...
    assert "not Agent libOS 1.6.0 release evidence" in text
...
def test_canonical_release_docs_cover_artifacts_and_clean_install_readback() -> None:
    ...
    assert "dist/agent_libos-1.6.0-py3-none-any.whl" in development
    assert "dist/agent_libos-1.6.0.tar.gz" in development
...
def test_gui_dependency_baseline_is_current_and_lockfile_aligned() -> None:
    ...
    assert locked_root["version"] == "1.6.0"
...
def test_release_workflow_preserves_and_clean_installs_validated_artifacts() -> None:
    ...
    assert parsed["env"] == {
        "RELEASE_WHEEL": "dist/agent_libos-1.6.0-py3-none-any.whl",
        "RELEASE_SDIST": "dist/agent_libos-1.6.0.tar.gz",
